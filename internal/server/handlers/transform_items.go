...
func (t *Transform) loadVideoMedias(ctx context.Context, videoListID, videoEpisodeID int64) ([]videoMediaRow, error) {
	query := `SELECT uuid, name, COALESCE(file_size,0), COALESCE(file_second,0),
			COALESCE(CAST(file_matadata AS CHAR),''), COALESCE(file_container,''),
			COALESCE(CAST(file_chapters AS CHAR),''), COALESCE(path_type,'')
...
func (t *Transform) UserCanDownload(ctx context.Context, userID int64) bool {
	...
	_ = t.db.QueryRowContext(ctx,
		"SELECT is_can_down FROM user WHERE id = ? LIMIT 1", userID,
	).Scan(&v)
	return v.Bool
}
